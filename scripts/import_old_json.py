...
def _get_or_create_group(db, name):
    if not name:
        return None
    for g in db.get_project_groups():
        if g["name"] == name:
            return g["id"]
    return db.add_project_group(name)
...
def import_one_file(db, json_path):
    ...
    # The old tool's own "project_name" field is the SAME across every
    # section file of one overall project (e.g. every Quba Mosque section
    # file says "QUBA MOSQUE EXPANSION PROJECT") — using it as the group
    # means all those sections are already grouped together the moment
    # they're imported, ready to be selected as one on the Dashboard/AI
    # Assistant, with no manual tagging needed afterward.
    group_id = _get_or_create_group(db, data.get("project_name", "") or "")
